...
export class USBManager {
    ...
    #winboat: Winboat | null = null;
    composeUsbDevices = ref<USBDevice[]>([])
    availableUsbDevices: USBDevice[] = [];

    constructor() {
        if (USBManager.instance) return USBManager.instance;

        this.#winboat = new Winboat();
    ...
    */
    async extractUSBFromDockerArgs(dockerArgs: string): Promise<USBDevice[]> {
        const usbDevices = [];
        ...
        for (const argument of dockerArgs.trimEnd().split("\n")) {
            ...
            try {
                ...
                usbDevices.push(this.parseLSUSBLine(rawUsbData.split("\n")[0]));
            } catch (e) {
    ...
    async fetchUSBDevices(options?: USBDeviceFetchOptions): Promise<USBDevice[]> {
        ...
        for (const rawDeviceData of rawUsbDevices) {
            const currentDevice = this.parseLSUSBLine(rawDeviceData);
     ...
     */
    serializeUSBDevices(devices: USBDevice[]): string {
     ...
     */
    parseLSUSBLine(line: string): USBDevice {
